...
function findBeltNearBy(belt) {
  ...
  while (queue.length > 0) {
    ...
    if (
      upY < rowCount &&
      upX < colCount &&
      storageStore.conveyorLocations[upY][upX] !== null
    ) {
    ...
    if (
      downY < rowCount &&
      downX < colCount &&
      storageStore.conveyorLocations[downY][downX] !== null
    ) {
    ...
    if (
      leftY < rowCount &&
      leftX < colCount &&
      storageStore.conveyorLocations[leftY][leftX] !== null
    ) {
    ...
    if (
      rightY < rowCount &&
      rightX < colCount &&
      storageStore.conveyorLocations[rightY][rightX] !== null
    ) {
...
function getBeltByPosition(grid_x, grid_y) {
  const storageStore = useStorageStore();
  const belt_id_in_out =
    storageStore.conveyorLocations?.[grid_y - 1]?.[grid_x - 1];
